...
function reanimateColors() {
    let now = Date.now(),
        ...
        rarehexa_transition = (now / 6000) % 1,
    ...
    animatedColor.rarehexa = hslToRgb(rarehexa_transition, 0.5, 0.75);
    ...
    animatedColor.peru = [peru_red, peru_white, peru_green, peru_white, peru_red][five_bars];
}
function getColor(colorNumber) {
    switch (colorNumber) {
        ...
        case "darkBlueSky":
            return "#97C4C9";
        case 79:
            return animatedColor.peru;
        case "legendaryTeal":
            return animatedColor.rarehexa;
        default:
            return "#00000000";
    }
}
    ...
    for (let i = 0; i < m.turrets.length; i++) {
        ...
        if (!t.layer) {
            let ang = t.direction + t.angle + rot,
                ...
                facing = 0
            if (mirrorMasterAngle && drawingEntities) {
...
function drawSelfInfo(spacing, alcoveSize, max) {
    ...
    //Draw the health number of leader
    drawText("Health: " + util.handleLargeNumber(gui.health), x + len / 5, y + height / 2, height - 2, color.guiwhite, "center", true);
}
