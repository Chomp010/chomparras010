...
class Entity extends EventEmitter {
    ...
    life() {
        ...
        // React
         ...
         this.move();
         this.face();
        } else {this.control.fire = false; this.control.main = false; this.control.alt = false;};
    ...
    define(set) {
        ...
        /*if (set.NAME_TYPE != null) {
            if (set.NAME_TYPE == "gemini") this.name = this.master.name;
        } else*/ if (set.NAME != null) this.name = set.NAME;
        ...
        if (set.BODY != null) {
            ...
            if (set.BODY.KNOCKBACK_MULTIPLIER != null) this.knockbackMultiplier = set.BODY.KNOCKBACK_MULTIPLIER;
            if (set.BODY.STUN_MULTIPLIER != null) this.stunMultiplier = set.BODY.STUN_MULTIPLIER;
            if (set.BODY.RAGE_MULTIPLIER != null) this.rageMultiplier = set.BODY.RAGE_MULTIPLIER;
            if (set.BODY.HETERO != null) this.heteroMultiplier = set.BODY.HETERO;
            this.refreshBodyAttributes();
        }
        /*
        if (set.INTERACT != null) {
            if (set.INTERACT.HURT != null) this.ACCELERATION = set.INTERACT.HURT;
            if (set.INTERACT.DEATH != null) this.SPEED = set.INTERACT.DEATH;
        }*/
    ...
    refreshBodyAttributes(range = true) {
        let speedReduce = Math.pow(this.size / (this.coreSize || this.SIZE), 1);
        this.acceleration = (c.runSpeed * this.ACCELERATION) / speedReduce;
        if (this.settings.reloadToAcceleration) this.acceleration *= this.skill.acl;
        this.topSpeed = (c.runSpeed * this.SPEED * this.skill.mob * this.rageRemaining) / speedReduce;
        ...
        if (range == true) if (!this.settings.dieAtRange || !this.range) this.range = this.RANGE;
    ...
    move() {
        ...
        case 'whirl':
            ...
            let ceno = (this.velocity.length && this.topSpeed) ? 1 * (this.topSpeed / 5) / (this.velocity.length / 5) : 0 ;
            let cenotarg;
            
            if (ceno >= 0.2 && ceno <= 5) {
            cenotarg = this.defaultSize * ceno;
            } else if (ceno > 5) {
            cenotarg = this.defaultSize * 5;
            } else {cenotarg = this.defaultSize * 0.2;}

            if (cenotarg) { if (this.SIZE < cenotarg + 0.75) {this.SIZE += 0.75;} else if (this.SIZE > cenotarg - 0.75) {this.SIZE -= 0.75;}}
            ...
            if (gactive) {
                ...
                if (l > this.size * 2) {
                    let XvelDesired = this.topSpeed * g.x / l,
                        YvelDesired = this.topSpeed * g.y / l;
                    engine = {                
                        x: (XvelDesired - this.velocity.x) * a / 20,
                        y: (YvelDesired - this.velocity.y) * a / 20,
                    };
                } else {
                if (this.velocity.length < this.topSpeed) {
                    engine = {
            ...
            let chain = new Vector(this.x - this.source.x, this.y - this.source.y);
            let dist = chain.length;
            let difference = this.size + this.source.size - dist;
    if (difference <= -(this.size + this.source.size) * 10) {
                this.kill();
                this.destroy();
    } else if (difference <= -(this.size + this.source.size) * 5) {
        this.velocity.x = difference * chain.x / dist / 5 ;
        this.velocity.y = difference * chain.y / dist / 5 ;
    } else if (difference <= 0) {
        this.velocity.x += difference * chain.x / dist / 50;
        this.velocity.y += difference * chain.y / dist / 50;
    }
                break;
            case "withMaster":
    ...
    face() {
        ...
        switch (this.facingType) {
            ...
            case "toTarget":
                ...
                break;
            case "sourceToTarget":
                if (this.source.facing) this.facing = this.source.facing;
                break;
            case "locksFacing":
    ...
    contemplationOfMortality() {
        if (this.invuln) {
            this.invulnTimer -= 1 / roomSpeed;
            if (this.invulnTimer < 1) { this.invuln = false; }
            this.damageRecieved = 0;
            this.stunRecieved = 0;
            this.rageRemaining = 1;
            this.isStunned = false;
        ...
        this.health.damageTaken = this.health.max - this.health.amount;
        // Apply Stun
        if (this.stunRecieved) {
            let stunPower = this.stunRecieved;
            this.stunRemaining += stunPower / roomSpeed;
        }
        // Being Stunned
        if (this.stunRemaining > 0) {
            this.isStunned = true
            this.stunRemaining -= 1 / roomSpeed;
        } else {
            this.isStunned = false;
            this.stunRemaining = 0;
        }
        //APPLY RAGE
        if (this.rageMultiplier >= 1) {
            this.rageRemaining = 1 + (this.rageMultiplier * this.health.damageTaken / this.health.max);
        } else {
            this.rageRemaining = 1;
        }
        if (this.damageRecieved && this.rageMultiplier >= 1) {
            this.refreshBodyAttributes(false);
        }
        this.stunRecieved = 0;
        this.damageRecieved = 0;
        // Check for death
