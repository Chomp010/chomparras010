    ...
    if (doDamage) {
        ...
        if (!bail) {
            ...
            n.damageRecieved += __m * (__m > 0
              ...
              if (my.damageRecieved) my.stunRecieved += my.damageRecieved * n.stunMultiplier;
              if (n.damageRecieved) n.stunRecieved += n.damageRecieved * my.stunMultiplier;
        }
    }
    /************* DO MOTION ***********/
    if (nIsFirmCollide < 0) {
        ...
        let spring = 2 * Math.sqrt(savedHealthRatio._me * savedHealthRatio._n) / roomSpeed,
            ...
            modifiers = {
                _me: c.KNOCKBACK_CONSTANT * n.knockbackMultiplier * my.pushability / my.mass * deathFactor._n,
                _n: c.KNOCKBACK_CONSTANT * my.knockbackMultiplier * n.pushability / n.mass * deathFactor._me,
            };
...
function reflectCollide(wall, bounce) {
    ...
    if ((left && !top && !bottom) || (leftExposed && !topExposed && !bottomExposed)) {
        bounce.accel.x -= (bounce.x + bounce.size - wall.x + wall.size * wall.knockbackMultiplier) * bounceBy
    } else if ((right && !top && !bottom) || (rightExposed && !topExposed && !bottomExposed)) {
        bounce.accel.x -= (bounce.x - bounce.size - wall.x - wall.size * wall.knockbackMultiplier) * bounceBy
    } else if ((top && !left && !right) || (topExposed && !leftExposed && !rightExposed)) {
        bounce.accel.y -= (bounce.y + bounce.size - wall.y + wall.size * wall.knockbackMultiplier) * bounceBy
        ...
        if (!x || !y) {
            ...
            bounce.accel.x += point.x * force * wall.knockbackMultiplier
            bounce.accel.y += point.y * force * wall.knockbackMultiplier
    ...
    if (intersected) {
        if ((bounce.type === 'tank' || bounce.type === 'miniboss') && wall.DAMAGE <= 1) {
