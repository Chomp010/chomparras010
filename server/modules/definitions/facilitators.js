...
exports.makeAuto = (type, name = -1, options = {}) => {
    ...
    if (options.controllers != null) {
        turret.controllers = options.controllers;
    } else { 
        turret.controllers = ["nearestDifferentMaster"] 
    }
    ...
    let autogun = {
        ...
        TYPE: [
            turret.type,
            {
                CONTROLLERS: turret.controllers,
...
exports.addAura = (damageFactor = 1, sizeFactor = 1, auraColor, options = {}) => {
    if (options.knockbackFactor != null) {knockbackFactor = options.knockbackFactor} else knockbackFactor = 0;
    let isHeal = damageFactor < 0;
    let auraType = knockbackFactor != 0 ? knockbackFactor > 0 ? "repelAura" : "gravityAura" : isHeal ? "healAura" : "aura";
